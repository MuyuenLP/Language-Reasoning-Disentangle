...
class Evaluator:
    ...
    def load_results(self):
        ...
        for data_path in self.subtasks_data_path:
            ...
            if "rep_fidelity" in subtask_data.columns:
                subtask_data.drop(['rep_fidelity', 'rea_fidelity'], axis=1, inplace=True)
                
            if "weighted_score" in subtask_data.columns:
                subtask_data.drop(['weighted_score'], axis=1, inplace=True)

            if "rouge_1" in subtask_data.columns:
                subtask_data.drop(['rouge_1', "rouge_2", "rouge_l", "bleu"], axis=1, inplace=True)
